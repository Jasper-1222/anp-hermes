...
状态：前十个 OpenSpec 变更已完成并归档；项目进入社区参考实现候选收尾审计阶段
...
当前剩余工作不再是“补齐最小协议链路”，而是面向社区贡献前的收尾：生产部署说明、上游协作、可选真实 LLM 验证、限流/持久化审计和示例客户端。
...
- 审计记录默认包含 caller DID、request id、tool name、状态、错误类别和耗时，不记录完整参数或完整结果。
...
该测试依赖 `~/.hermes/config.yaml` 中的 provider 和对应 API key。缺少配置或密钥时，应记录为条件未执行或按预期 skip，不应描述为已通过。

## 4. 仍需谨慎的边界

### 4.1 Bearer token 后续请求完整验收

当前实现已经支持成功认证时返回 `Authentication-Info`，但仍建议后续单独 OpenSpec 化 Bearer token 后续请求完整验收，覆盖：

- 首次签名请求返回 token。
- 后续 Bearer token 请求被接受。
- token 过期或无效时回退到签名挑战或返回安全错误。

### 4.2 ANP SDK resolver injection 上游协作

当前 resolver override 已收紧为本地 loopback 用途，生产默认走 DID WBA HTTPS 解析。中长期仍建议推动 ANP Python SDK 提供正式 resolver injection API，减少插件对 SDK 全局 resolver 行为的封装压力。

### 4.3 生产部署指南

当前 README 已说明本地和测试床配置，但生产部署还可以单独补一份指南，覆盖：

- HTTPS 反向代理。
- DID path 路由。
- Hermes gateway systemd/docker 部署。
- `ANP_ALLOW_ALL_USERS` 禁用策略。
- tool RPC allowlist 审批建议。
- 日志、备份和密钥轮换。

### 4.4 限流与审计持久化

tool RPC 已有默认关闭、授权、错误映射和审计元数据。公开服务前仍建议新增：
...
建议按以下顺序继续 OpenSpec 化，每项独立成 change：

1. `document-production-deployment`
   - 产出生产部署指南、HTTPS path DID 路由说明、密钥与日志运维建议。
...
下一步不建议继续做宽泛重构。更好的路径是先完成 `review-community-readiness` 收尾审计，确认文档和验证矩阵稳定，然后进入代码评审、PR 或发布准备。后续新增能力应继续保持 OpenSpec 小步推进。