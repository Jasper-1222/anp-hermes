# anp-core-binding Specification

## Purpose

定义 `anp-agent` 插件对 ANP Core Binding v1 的最小兼容契约：通过 `anp.get_capabilities` 暴露运行时能力，支持 `params.meta/body` envelope，并对 profile、security profile 与参数形态返回机器可读错误。
