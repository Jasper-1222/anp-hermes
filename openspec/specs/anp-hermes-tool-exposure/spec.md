# anp-hermes-tool-exposure Specification

## Purpose

定义 `anp-agent` 插件通过可选 tool RPC 安全暴露 Hermes tools 的能力边界：默认关闭，只有在显式配置 allowlist、denylist 与 caller DID 授权后，才可通过 `hermes.tool.*` JSON-RPC 方法调用低风险 Hermes tools。
