...
class ANPBridgeError(Exception):
    """桥接层结构化错误。"""

    def __init__(self, message: str, rpc_code: int = -32603) -> None:
...
class ANPBridge:
    ...
    async def call(
        ...
        Args:
            rpc_id: 客户端 JSON-RPC 请求 id，用于响应回显与元数据追踪。
        ...
        Raises:
            ANPBridgeError: 请求无法提交、超时、取消或 pending 数量已达上限。
        """
        self._cleanup_expired()

        if len(self._pending) >= self._max_pending:
            raise ANPBridgeError("服务繁忙，请稍后重试")

        request_id = self._next_request_id()
        event = MessageEvent(
            text=params.get("message", ""),
            message_id=request_id,
            source=_build_source(request_id, caller_did),
            metadata={
                "anp_rpc_id": rpc_id,
                "anp_request_id": request_id,
                "anp_method": method,
                "anp_params": params,
        ...
        self._pending[request_id] = _PendingFuture(future=future)
        ...
        try:
            ...
            future.cancel()
            raise ANPBridgeError("无法将请求提交给 Hermes") from exc
        ...
        try:
            ...
            logger.warning("request_id=%s 等待结果超时", request_id)
            self._pending.pop(request_id, None)
            if not future.done():
                future.cancel()
            raise ANPBridgeError("请求处理超时") from exc
        except asyncio.CancelledError as exc:
            logger.debug("request_id=%s 的 Future 被取消", request_id)
            self._pending.pop(request_id, None)
            if future.cancelled():
                raise ANPBridgeError("请求已取消") from exc
            future.cancel()
            raise
        except Exception as exc:
            logger.exception("等待 request_id=%s 结果时发生异常: %s", request_id, exc)
            self._pending.pop(request_id, None)
            raise ANPBridgeError("请求处理过程中发生内部错误") from exc

    def set_result(self, request_id: str, content: str) -> bool:
        """根据服务端内部 request id 设置对应 pending Future 的结果。

        Args:
            request_id: 服务端内部 request id。
        ...
        if entry is None:
            logger.debug("request_id=%s 无对应 pending future", request_id)
            return False
    ...
    def _cleanup_expired(self) -> None:
        ...
        for request_id in expired_keys:
            ...
            logger.debug("清理过期 pending future: request_id=%s", request_id)
