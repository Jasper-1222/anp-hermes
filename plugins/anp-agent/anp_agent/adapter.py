...
class ANPAdapter(BasePlatformAdapter):
    ...
    async def send(
        ...
        request_id = chat_id[4:]
        if not request_id:
            return SendResult(success=False, error="empty request_id")
        if not self._bridge.set_result(request_id, content):
            return SendResult(success=False, error="request_id not found or already resolved")
