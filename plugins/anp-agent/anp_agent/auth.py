...
# 当前生效的 wrapper 配置；非空 base_url 不允许被不同配置静默覆盖
_resolver_config = {
    "timeout": _DEFAULT_DID_RESOLVE_TIMEOUT,
    "base_url": None,
    "verify_ssl": True,
}
...
def _is_loopback_host(hostname: str | None) -> bool:
    """判断 resolver override hostname 是否为本机 loopback。"""
    if hostname is None:
        return False
...
def _parse_resolver_base_url(raw: str | None) -> tuple[str | None, bool]:
    """解析并校验 resolver base URL，返回 (base_url, verify_ssl)。"""
    ...
    if parsed.scheme not in {"http", "https"} or not parsed.netloc:
        raise ValueError(f"{_DID_RESOLVER_BASE_URL_ENV} 必须是 http(s) base URL")
    if not _is_loopback_host(parsed.hostname):
        raise ValueError(f"{_DID_RESOLVER_BASE_URL_ENV} 仅允许 loopback base URL")
    return base_url, parsed.scheme == "https"


def _sanitize_resolver_value_error(exc: ValueError) -> str:
    """将 resolver 的 ValueError 转成不含内部 URL 的安全分类消息。"""
    message = str(exc).lower()
    if any(keyword in message for keyword in ("did document", "proof", "binding")):
        return "Invalid DID document: validation failed"
    return "Invalid DID document: resolver validation failed"


def _extract_success_headers(response_headers: Any) -> dict[str, str]:
    """从 verifier 成功响应头中提取允许转发的响应头。"""
    if not isinstance(response_headers, dict):
        return {}
    return {
        "Authentication-Info": value
        for name, value in response_headers.items()
        if name.lower() == "authentication-info" and isinstance(value, str) and value
    }
...
def _make_resolver_wrapper(resolve_fn):
    ...
    async def _resolver_wrapper(did: str, verify_proof: bool = False):
        ...
        verify_ssl = _resolver_config["verify_ssl"]
        try:
            if base_url is not None:
                coro = did_resolver_module.resolve_did_document(
                    did,
            ...
            raise DidWbaVerifierError(
                _sanitize_resolver_value_error(exc),
                status_code=401,
            ) from exc
...
class ANPAuth:
    ...
    def __init__(self, identity: ANPIdentity) -> None:
        ...
        self._did_resolver_base_url, self._did_resolver_verify_ssl = _parse_resolver_base_url(
            os.environ.get(_DID_RESOLVER_BASE_URL_ENV)
        )
    ...
    def _patch_resolver(self) -> None:
        ...
        """
        existing_base_url = _resolver_config.get("base_url")
        if (
            existing_base_url
            and self._did_resolver_base_url
            and existing_base_url != self._did_resolver_base_url
        ):
            raise RuntimeError("DID resolver override conflicts with existing process resolver")

        _resolver_config["timeout"] = self._did_resolve_timeout
        _resolver_config["base_url"] = self._did_resolver_base_url
        _resolver_config["verify_ssl"] = self._did_resolver_verify_ssl

        current = did_wba_verifier_module.resolve_did_wba_document
        if getattr(current, "_anp_auth_wrapper", False):
            # 已经是 wrapper，只需更新全局配置
            logger.debug(
                "DID resolver wrapper 已存在，更新配置: timeout=%.1f, base_url=%s, verify_ssl=%s",
                self._did_resolve_timeout,
                self._did_resolver_base_url if self._did_resolver_base_url else "(none)",
                self._did_resolver_verify_ssl,
            )
            return
        ...
        logger.debug(
            "DID 文档解析超时已设置为 %.1f 秒，base_url_override=%s, verify_ssl=%s",
            self._did_resolve_timeout,
            self._did_resolver_base_url if self._did_resolver_base_url else "(none)",
            self._did_resolver_verify_ssl,
        )
    ...
    async def authenticate(
        ...
        try:
            ...
            return AuthenticationResult(
                ...
                headers=_extract_success_headers(result.get("response_headers")),
            )
...
def _parse_timeout(raw: Any) -> float:
    """解析 DID 解析超时配置，非法值回退默认值，超大值限制到上限。"""
    try:
        timeout = float(raw)
    ...
    if timeout <= 0:
        logger.warning(
            "ANP_DID_RESOLVE_TIMEOUT 必须大于 0 (%r)，回退默认值 %s",
            raw,
            _DEFAULT_DID_RESOLVE_TIMEOUT,
        )
        return float(_DEFAULT_DID_RESOLVE_TIMEOUT)

    if timeout > _MAX_DID_RESOLVE_TIMEOUT:
        logger.warning(
            "ANP_DID_RESOLVE_TIMEOUT 超过上限 (%r)，使用上限 %s",
            raw,
            _MAX_DID_RESOLVE_TIMEOUT,
        )
        return float(_MAX_DID_RESOLVE_TIMEOUT)
