...
**ANP × Hermes 技术验证 Demo**：通过 Hermes 插件机制，让 [Hermes](https://github.com/NousResearch/hermes-agent) 智能体成为可被发现、认证和调用的 ANP 服务智能体，为其他智能体框架接入 ANP 提供可参考的实现样板。

[ANP（Agent Network Protocol）](https://github.com/agent-network-protocol) 是智能体之间发现、认证与调用的开放协议。本仓库提供 Hermes 服务端插件与 ANP 调用端 skill，用一个可运行、可测试的端到端示例，打通从 Agent 发现、DID WBA 身份认证到 JSON-RPC 对话的完整链路。
